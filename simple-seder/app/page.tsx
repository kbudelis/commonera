...
import { coverOptions, foundationalPassages, quoteCatalog, sourceCatalog, themeDescriptions, themeLabels } from "@/content/pack";
import { generateHaggadah, mergeModelEnhancement, type ModelEnhancement } from "@/lib/generator";
import { defaultProfile, type GenerationProfile, type HaggadahDocument, type HaggadahSection, type ThemeId } from "@/lib/types";
...
function Cover({ document, coverId, compact = false }: { document: HaggadahDocument; coverId: string; compact?: boolean }) {
  const cover = coverOptions.find((item) => item.id === coverId) ?? coverOptions[0];
  const date = formatSederDate(document.profile.sederDate);
  return <div className={`cover ${compact ? "compact" : ""} ${cover.ink}`}><img className="cover-bg" src={assetPath(cover.image)} alt="" style={{ filter: cover.filter, objectPosition: cover.position }}/><span className="cover-shade"/>
    ...
    <div className="cover-bottom"><span>{document.profile.hostName || "Made for your table"} · {date}</span><strong>PassoverForBeginners.com</strong></div>
  </div>;
}
...
export default function Home() {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      const coverResponse = await fetch(assetPath(cover.image));
      ...
      pdf.addImage(coverDataUrl, cover.image.endsWith(".webp") ? "WEBP" : "PNG", (pageW - coverW) / 2, (pageH - coverH) / 2, coverW, coverH, undefined, "FAST");
      ...
      pdf.setFont("helvetica", "bold"); pdf.setFontSize(9); pdf.text("PassoverForBeginners.com", pageW / 2, 699, { align: "center" });
      for (const section of document.sections) {
        let y = 66;
        const addContentPage = () => { pdf.addPage(); pdf.setTextColor(35, 53, 51); pdf.setFont("helvetica", "normal"); pdf.setFontSize(8); pdf.text("PassoverForBeginners.com", pageW - margin, pageH - 28, { align: "right" }); y = 64; };
      ...
      const usedPassageIds = new Set(document.sections.flatMap((section) => section.passageIds));
      const usedSources = sourceCatalog.filter((source) => usedSourceIds.has(source.id)).map((source) => {
        const passages = foundationalPassages.filter((passage) => passage.sourceId === source.id && usedPassageIds.has(passage.id));
        const passageNote = passages.length ? ` Passages used: ${passages.map((passage) => `${passage.locator} (${passage.treatment})`).join("; ")}.` : "";
        return { title: source.title, creator: source.creator, url: source.url, rights: `${source.rights}.${passageNote}` };
      });
      ...
      pdf.setFontSize(6); pdf.text("PassoverForBeginners.com", pageW - margin, pageH - 28, { align: "right" });
  ...
  return <main>
    <nav className="topbar"><a className="brand" href="#top"><span>פ</span> Passover for Beginners</a><div><a href="#how">How it works</a><button className="nav-cta" onClick={() => builderRef.current?.scrollIntoView({ behavior: "smooth" })}>Build yours</button></div></nav>
    ...
    <section className="builder" ref={builderRef}><div className="builder-head"><p className="eyebrow">Let’s make your Haggadah</p><h2>What kind of night are you planning?</h2><p>There are no wrong answers. You can change everything later.</p></div>
      ...
      <div className="form-card">
        <div className="intake-progress" aria-label="Builder progress">{["The table", "The feeling", "Finishing touches"].map((label, index) => <button key={label} type="button" className={intakeStep === index + 1 ? "active" : intakeStep > index + 1 ? "done" : ""} onClick={() => setIntakeStep(index + 1)} aria-current={intakeStep === index + 1 ? "step" : undefined}><span>{intakeStep > index + 1 ? "✓" : index + 1}</span>{label}</button>)}</div>
        <div className="intake-step" key={intakeStep}>
        ...
        {intakeStep === 3 && <><p className="step-intro">Personalize the cover, or leave these blank and start with our defaults.</p><fieldset className="context"><legend>Add the finishing details</legend><div className="input-grid"><label>Host or family name<input value={profile.hostName} onChange={(e) => setProfileKey("hostName", e.target.value)} placeholder="The Cohen family" /></label><label>Seder date<input type="date" value={profile.sederDate} onChange={(e) => setProfileKey("sederDate", e.target.value)} /></label><label className="wide">Cover quote <small>(optional)</small><input value={profile.coverQuote} onChange={(e) => setProfileKey("coverQuote", e.target.value)} placeholder="A line you want guests to carry with them" /></label></div><label className="switch"><input type="checkbox" checked={profile.antiZionist} onChange={(e) => setProfileKey("antiZionist", e.target.checked)} /><i/><span><strong>Use peace-focused non-Zionist language</strong><small>Uses peace-focused non-Zionist framing and replaces “Next year in Jerusalem” with “Next year in peace.”</small></span></label></fieldset></>}
        </div>
        <div className="intake-nav">{intakeStep > 1 && <button type="button" className="back" onClick={() => setIntakeStep((step) => step - 1)}>← Back</button>}<span/>{intakeStep < 3 ? <button type="button" className="primary" onClick={() => setIntakeStep((step) => step + 1)}>Continue <span>→</span></button> : <button className="primary large" onClick={build} disabled={busy}>{busy ? "Building…" : "Build my Haggadah"} <span>→</span></button>}</div>
      </div>
      <aside className={`intake-preview ${previewCover.ink}`}><img src={assetPath(previewCover.image)} alt=""/><span className="preview-shade"/><div><p className="eyebrow">Your seder is taking shape</p><h3>{profile.hostName ? `${profile.hostName}’s Haggadah` : "A Haggadah for your table"}</h3><p>{profile.coverQuote || "We tell the story so freedom can become practice."}</p><ul><li>{profile.length} minutes</li><li>{choices.audience.find(([id]) => id === profile.audience)?.[1]}</li><li>{profile.themes.length ? profile.themes.map((id) => themeLabels[id]).join(" · ") : "A welcoming classic"}</li></ul><small>{previewCover.name}</small></div></aside>
      </div>
    </section>
    {document && <section className={`result type-${document.profile.typography}`} id="result">
      ...
      {tab === "invitation" && <div className="invite-wrap" id="panel-invitation" role="tabpanel" aria-labelledby="tab-invitation"><div className="invitation"><p className="eyebrow">You’re invited</p><span className="invite-mark">פֶּסַח</span><textarea value={document.invitation} rows={6} onChange={(e) => changeDocument((current) => ({ ...current, invitation: e.target.value }))} aria-label="Invitation text"/><b>PassoverForBeginners.com</b></div><button className="primary" onClick={copyInvitation} disabled={busy}>Copy invitation</button></div>}
    </section>}
    <footer><a className="brand" href="#top"><span>פ</span> Passover for Beginners</a><p>An open, welcoming way into an ancient story.</p><b>PassoverForBeginners.com</b></footer>
  </main>;
}
