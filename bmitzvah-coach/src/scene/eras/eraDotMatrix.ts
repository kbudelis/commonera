...
export const eraDotMatrix: EraDef = {
  ...
  create({ inkTexture, pbr: _pbr }) {
    ...
    // The printer the paper feeds from.
    const charcoal = track(new MeshStandardNodeMaterial({ color: '#3d3b3f', roughness: 0.7 }));
    const printer = new Mesh(track(new BoxGeometry(1.62, 0.3, 0.5)), charcoal);
    printer.position.set(0, 0.9, -0.24);
    const platen = new Mesh(
      track(new CylinderGeometry(0.055, 0.055, 1.5, 20)),
      track(new MeshStandardNodeMaterial({ color: '#232125', roughness: 0.5 })),
    );
    platen.rotation.z = Math.PI / 2;
    platen.position.set(0, 0.74, -0.13);
    group.add(paper, printer, platen);
